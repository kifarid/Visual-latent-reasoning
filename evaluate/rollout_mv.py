...
def generate_images(args, unknown_args):
    ...
    print(f"Using validation config: {args.val_config if args.val_config is not None else config}")
    # Get the dataset (for conditioning)
    num_condition_frames = config.data.params.validation.params.num_frames - 2
    ...
    for batch_idx, _ in enumerate(progress_bar):
        ...
        if isinstance(data_batch, dict):
            ...
            raymaps = data_batch.get('raymaps', None).cuda() if 'raymaps' in data_batch else None
        elif hasattr(data_batch, 'cuda'):
            x = data_batch.cuda()
            raymaps = None

        assert x.shape[2] == 3 and x.shape[1] == num_frames, f"Data batch should have shape (bs, {num_frames}, 3, h, w), got {x.shape}"
        cond_x = x[:, :num_condition_frames]
        print(f"Generating {args.num_gen_frames} frames for sample {sample_idx} with shape {cond_x.shape}"
              f" and raymaps {raymaps.shape if raymaps is not None else None}"
              f" (batch size {cond_x.size(0)})"
              f" cond_x: {cond_x.shape}, x: {x.shape}")
        latents, gen_frames = model.roll_out(x_0=cond_x, num_gen_frames=args.num_gen_frames, latent_input=False, eta=args.eta, NFE=args.num_steps, sample_with_ema=args.evaluate_ema, num_samples=cond_x.size(0), raymaps=raymaps)
        ...
        for sample_in_batch_idx in range(x.shape[0]):
            ...
            gen_frames_orig = gen_frames[sample_in_batch_idx]

            f_total = gen_frames_orig.shape[0]  # 16
            v = 2
            assert f_total % v == 0, "Frames not divisible by v"

            # rearrange
            gen_frames_rearranged = rearrange(gen_frames_orig, '(f v) c h w -> f c h (v w)', v=v)

            for f in range(len(gen_frames_rearranged)):
                save_image((gen_frames_rearranged[f]+1.0)/2.0, os.path.join(subfolder_path_fake, f"mv_frame_{f:04d}.jpg"))

            # gifs multiview
            imageio.mimsave(os.path.join(subfolder_path_gifs, f"sequence_{sample_idx:04d}.gif"), [np.array(Image.open(os.path.join(subfolder_path_fake, f"mv_frame_{f:04d}.jpg"))) for f in range(len(gen_frames_rearranged))], fps=7, loop=0)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_gen_frames", type=int, default=30, help="Number of frames to generate (roll-out length)")
