...
#################################################################################
#                       Conditioning Embedding Functions                        #
#################################################################################
    ...
    def forward(self, x, train=False):
        """ Forward pass through the embedding module
            :param:
                ...
                train -> bool: Whether to use training mode (default: False)
            :return
                torch.Tensor: Output of the embedding module
        """
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        concept_dim=[2,2,2],
        num_concepts=3,
        concept_discrete=[True, True, True],
        ...
        self.concept_discrete = concept_discrete
        self.class_dropout_prob = class_dropout_prob
        self.concept_discrete = concept_discrete
        self.y_embedder = nn.ModuleList()
        for i in range(num_concepts):
            if concept_discrete[i]:
                self.y_embedder.append(LabelEmbedder(self.concept_dim[i], hidden_size, class_dropout_prob))
            else:
                self.y_embedder.append(EmbedFC(self.concept_dim[i], hidden_size))
        
        self.conditional = num_concepts > 0   
    ...
    def initialize_weights(self):
        ...
        for i in range(self.num_concepts):
            if self.concept_discrete[i]:
                nn.init.normal_(self.y_embedder[i].embedding_table.weight, std=0.02)
            else:
                # Initialize the embedding table for continuous concepts
                self.y_embedder[i].apply(_basic_init)
    ...
    def forward(self, target, context, t, y=None, drop_label=None, return_latents=[], reg_tokens=None, early_exit=False):
        """
        #TODO this should be updated to match signature
        Forward pass of DiT.
        ...
        if y is not None and self.conditional:
            label_embs = [self.y_embedder[i](v, self.training) for i, (_, v) in enumerate(y)]
            if label_embs:
                c = c + torch.stack(label_embs, dim=0).sum(dim=0)  # (N, D)
...
class FlexDiT(DiT):
    ...
    def forward(self, target, context, t, frame_idxs = None, reg_tokens=None, return_latents=[], return_regs=[], y=None, early_exit=False):
        ...
        if y is not None and self.conditional:
            label_embs = [self.y_embedder[i](v, self.training) for i, (_, v) in enumerate(y)]
            if label_embs:
                c = c + torch.stack(label_embs, dim=0).sum(dim=0)
