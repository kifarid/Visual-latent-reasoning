...
class MVDiT(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            nviews: number of views for the input data.
    ...
    def forward(self, x, t, cond_signals=None, view_indicies=None):
        """
        Forward pass of DiT.
...
class MVSTDiT(MVDiT):
    ...
    def forward(self, x, t, cond_signals=None, t_indices=None):
        """
        Forward pass of DiT.
        ...
        #move the view embeddings to the number of tokens hw

        # Here, f = t * v. We want to keep t and move v next to hw.
        # Assume input shape is (b, f, hw, c) where f = t * v.
        # We need to reshape to (b, t, v, hw, c), then move v next to hw: (b, t, hw, v, c).
        # If t_indices is (N, t), we infer v = f // t.
        v = self.nviews
        t = f // v
        x = rearrange(x, 'b (t v) hw c -> b t (v hw) c', t=t, v=v)
        # subsample c
        c_trans = rearrange(c, 'b (t v) d -> b t v d', t=t, v=v)
        c_trans = c_trans.repeat_interleave(repeats=x.shape[-2]//v, dim=2)
        #possible C dimensions must start with B and end with C
        for block in (self.blocks):
            x = block(x, c_trans)
        
        #rearrange back to (b, f, hw, c)
        x = rearrange(x, 'b t (v hw) c -> b (t v) hw c', t=t, v=v)
