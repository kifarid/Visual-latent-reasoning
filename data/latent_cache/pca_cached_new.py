...
from data.vds_minimal_dataset import MinimalVDSLatentDataset
...
def main():
    ...
    ap.add_argument("--vds-path",
                    default="/work/dlclarge2/faridk-diff_force/datasets/Covla/h5/vds_splits_minimal.h5",
                    help="Path to VDS file with frames and latents.")
    ap.add_argument("--vds-split", default="train", help="Split name inside the VDS file")
    ap.add_argument("--latents-dtype", choices=["fp16", "fp32"], default="fp32",
                    help="Convert loaded latents to this dtype.")
    args = ap.parse_args()
    ...
    if not args.vds_path:
        raise ValueError("--vds-path must be provided (MinimalVDSLatentDataset is required)")

    ds = MinimalVDSLatentDataset(
        vds_path=args.vds_path,
        split=args.vds_split,
        num_frames=args.T,
        size=size,
    ...
    for batch in dl:
        for sample in batch:
            ...
            idx = samples_done
            info = None
            index_lookup = getattr(ds, "index_of", None)
            if callable(index_lookup):
                try:
                    info = index_lookup(idx)  # type: ignore[call-arg]
                except Exception:
                    info = None
            ...
            if frames_t is None:
                if info is None:
                    warnings.warn("No frame tensor available and index metadata missing; skipping sample.")
                    continue
            ...
            if info is not None:
                stem = Path(info["h5_path"]).stem
                key_name = info["key"]
                start_frame = info["start_frame"]
            else:
                stem = Path(args.vds_path).stem if args.vds_path else "dataset"
                video_id = sample.get("video_id")
                frame_idx_md = sample.get("frame_idx")
                if torch.is_tensor(video_id) and video_id.numel() > 0:
                    key_name = f"vid{int(video_id[0])}"
                else:
                    key_name = "vid"
                if torch.is_tensor(frame_idx_md) and frame_idx_md.numel() > 0:
                    start_frame = int(frame_idx_md[0])
                else:
                    start_frame = 0
            name = f"idx{idx:06d}_key={stem}:{key_name}_s{start_frame}.png"
