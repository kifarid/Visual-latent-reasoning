...
class MultiHDF5DatasetMultiFrameIdxMapping(Dataset):
    ...
    def __init__(self, size, hdf5_paths_file, num_frames, stored_data_frame_rate=5, frame_rate=5, aug='resize_center', scale_min=0.15, scale_max=0.5):
        ...
        # map each index to a specific (file, key, starting_frame)
        self.scan_h5_files()
...
class HorizontalShiftCrop:
    def __init__(self, size, shift_pixels=0):
        """
        size: int or (height, width)
        shift_pixels: positive to shift right, negative to shift left
        """
        self.size = size if isinstance(size, tuple) else (size, size)
...
class MultiHDF5DatasetMultiFrameFromJSONFrameRateWrapper(MultiHDF5DatasetMultiFrameFromJSON):
    def __init__(self, size, samples_json, num_frames, stored_data_frame_rate, frame_rate, num_samples=800):
        raise NotImplementedError('Use MultiHDF5DatasetMultiFrameFromJSON directly, it now supports frame_rate and stored_data_frame_rate parameters.')
        frame_rate_multiplier = frame_rate/stored_data_frame_rate
        self.frame_rate = frame_rate
        self.stored_data_frame_rate = stored_data_frame_rate
        super().__init__(size, samples_json, num_frames, stored_data_frame_rate, frame_rate, num_samples)
...
class MultiHDF5DatasetMultiFrameRandomizeFrameRate(MultiHDF5DatasetMultiFrameIdxMapping):
    """
    Enable the model to sample from multiple frame intervals (with weights).
    """
    def __init__(self, *, size, hdf5_paths_file, num_frames, stored_data_frame_rate, frame_rates_and_weights, **kwargs):
        ...
        super().__init__(size, hdf5_paths_file, num_frames, stored_data_frame_rate=stored_data_frame_rate, frame_rate=self.frame_rates[0], **kwargs)
...
class MultiHDF5DatasetMultiFrameFromPrecomputed(MultiHDF5DatasetMultiFrameIdxMapping):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.transform = None  # no transform, will fail if transform is attempted
    ...
    def scan_h5_files(self):
        ...
        for file in self.hdf5_files:
            keys = list(file.keys())
            for key in keys:
                # there might be multiple versions of the same video for different augmentations
                version_keys = list(file[key].keys())
                video_length = len(file[key][version_keys[0]])
                # we take every nth frame, as long as we can get num_frames frames after that
                max_frame_index = video_length - self.num_frames*self.frame_interval-1
                for i in range(0, max_frame_index + 1):
                    self.index_to_starting_frame_map.append((file, key, i))
    ...
    def get_images_and_indices(self, idx):
        if idx >= len(self.index_to_starting_frame_map):
            raise IndexError(f'Index {idx} out of range for dataset of length {len(self.index_to_starting_frame_map)}')
        file, key, start_frame = self.index_to_starting_frame_map[idx]
        random_version_key = random.choice(list(file[key].keys()))
        images = file[key][random_version_key][start_frame:start_frame+self.num_frames*self.frame_interval:self.frame_interval]
        return images, (file.filename, key, start_frame)
...
class MixtureDataset(Dataset):
    def __init__(self, dataset_configs, dataset_weights=None):
        self.datasets = [instantiate_from_config(config) for config in dataset_configs]
        self.dataset_lengths = [int(len(dataset)) for dataset in self.datasets]
        self.cumulative_lengths = np.cumsum(self.dataset_lengths)
        
        if dataset_weights is None:
            dataset_weights = np.ones(len(self.datasets)) / len(self.datasets)
        else:
            dataset_weights = np.array(dataset_weights)
        assert len(self.datasets) == len(dataset_weights), 'Number of datasets and weights must be equal'
        assert sum(dataset_weights) == 1, 'Weights must sum to 1'
        self.weights = dataset_weights / np.sum(dataset_weights)
        
        print(self)
        
    def __str__(self):
        s = f'MixtureDataset(weights={self.weights}, datasets=[\n'
    ...
    def __len__(self):
        return int(sum([len(dataset) for dataset in self.datasets]))
    
    def __getitem__(self, idx):
        dataset_index = np.random.choice(len(self.datasets), p=self.weights)
        dataset = self.datasets[dataset_index]
        item_index = idx - self.cumulative_lengths[dataset_index-1] if dataset_index > 0 else idx
