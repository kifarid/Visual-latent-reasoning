...
class LatentSources(NamedTuple):
...
def _resolve_latent_sources(
    sources: List[str],
    latents_dir: str,
    latents_pattern: str,
    require_all: bool,
) -> Optional[LatentSources]:
    mapping: Dict[int, str] = {}
    for idx, src in enumerate(sources):
        base = os.path.splitext(os.path.basename(src))[0]
        search_dir = latents_dir or os.path.dirname(src)
        pattern = os.path.join(search_dir, f"{base}{latents_pattern}")
        print(f"Searching for latents for source {idx}: pattern '{pattern}'")
        candidates = sorted(glob.glob(pattern))
        if not candidates:
            continue
        if len(candidates) > 1:
            print(
                f"Warning: multiple latent shards matched '{pattern}', using '{candidates[0]}'"
            )
        mapping[idx] = candidates[0]

    if require_all and len(mapping) != len(sources):
        missing = [sources[i] for i in range(len(sources)) if i not in mapping]
        raise FileNotFoundError(
            "Latent shards not found for some sources: " + ", ".join(missing)
        )

    if mapping and len(mapping) != len(sources) and not require_all:
        missing = [sources[i] for i in range(len(sources)) if i not in mapping]
        print(
            "Warning: skipping latents entirely; no matches for: "
            + ", ".join(missing)
        )
        return None
    ...
    for path in sorted(set(mapping.values())):
        with h5py.File(path, "r") as hf:
            if "latents" not in hf:
                raise KeyError(f"Latents dataset not found in {path}")
            ds = hf["latents"]
            shape = tuple(int(x) for x in ds.shape)
            shapes[path] = shape
            if row_shape is None:
                row_shape = shape[1:]
            elif shape[1:] != row_shape:
                raise ValueError(
                    f"Latents row shape mismatch: expected {row_shape}, found {shape[1:]} in {path}"
                )
            if dtype is None:
                dtype = ds.dtype
            elif ds.dtype != dtype:
                raise ValueError(
                    f"Latents dtype mismatch: expected {dtype}, found {ds.dtype} in {path}"
                )

    if dtype is None or row_shape is None:
        return None

    return LatentSources(mapping, shapes, dtype, row_shape)
...
def _create_split_vds(
    ...
    if latents is not None:
        layout_latents = h5py.VirtualLayout(shape=(N, *latents.row_shape), dtype=latents.dtype)
        ...
        for si, runs in vds_runs.items():
            ...
            for src_start, dst_start, length in runs:
                ...
                if layout_latents is not None:
                    lat_path = latents.by_index.get(si)
                    if lat_path is None:
                        raise KeyError(
                            f"Latents shard missing for source index {si} ({spath})"
                        )
                    lat_shape = latents.shapes[lat_path]
                    vsrc_lat = h5py.VirtualSource(lat_path, "latents", shape=lat_shape)
                    layout_latents[dst_slice, ...] = vsrc_lat[src_slice, ...]
    ...
    if layout_latents is not None:
        grp.create_virtual_dataset("latents", layout_latents, fillvalue=0)
...
def main(args: argparse.Namespace) -> None:
    ...
    latents = _resolve_latent_sources(
        sources,
        args.latents_dir,
        args.latents_pattern,
        args.require_latents,
    )
    ...
    with h5py.File(args.output, "w", libver="latest") as out:
        ...
        if latents is not None:
            out.attrs.update({
                "latents_pattern": args.latents_pattern,
                "latents_required": bool(args.require_latents),
                "latents_sources": np.array(
                    [latents.by_index.get(i, "").encode("utf-8") for i in range(len(sources))],
...
if __name__ == "__main__":
    ...
    lat = p.add_argument_group("Latents (optional)")
    lat.add_argument("--latents-dir", type=str, default="", help="Directory to search for latent shards (defaults to source dir)")
    lat.add_argument("--latents-pattern", type=str, default="_latents-facebook-dinov3-vitl16-pretrain-lvd1689m-head.h5", help="Suffix/pattern appended to source base name, e.g. '.latents.h5'")
    lat.add_argument("--require-latents", action="store_true", help="Fail if any shard is missing latents")
