...
class ModelREPA(pl.LightningModule):
    def __init__(self, *, tokenizer_config, generator_config, adjust_lr_to_batch_size=False, sigma_min=1e-5, timescale=1.0, enc_scale=4, warmup_steps=5000, min_lr_multiplier=0.1,num_pred_frames=1, sr_latents=[],
        ...
        self.predictor_proj_yol, self.predictor_norm_yol = self.build_projector(
            generator_config.params.hidden_size,
            hidden_dim=generator_config.params.hidden_size,
        )
    ...
    def sr_loss(self, pred_latents, post_latents, pred_yol=False):
        ...
        for i in range(len(pred_latents)):
            ...
            # turn on dino only for predictor 
    ...
    def training_step(self, batch, batch_idx):
        ...
        _, post_latents_yol = self.ema_vit(target, context, t.pow(5), return_latents=self.sr_latents, early_exit=True)
        ...
        # Total loss
        loss = self.recon_loss_weight * v_loss_mean + self.sr_loss_weight * sr_loss_teacher_mean + self.sr_yol_loss_weight * sr_loss_yol_mean
...
class ModelREPAIF(ModelREPA):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = (
            ...
            + self.sr_loss_weight * sr_loss_val_mean
        ...
        for layer_name, layer_val in sr_yol_layers.items():
            scalar_val = layer_val.item() if hasattr(layer_val, "item") else layer_val
            self.log(f"train/sr_yol/{layer_name}", scalar_val, prog_bar=False, logger=True, on_step=True, on_epoch=True, sync_dist=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("val/sr_loss_yol", sr_loss_yol_mean.item() if hasattr(sr_loss_yol_mean, "item") else sr_loss_yol_mean, prog_bar=False, logger=True, on_step=True, on_epoch=True, sync_dist=True)
        for layer_name, layer_val in sr_loss_layers.items():
            scalar_val = layer_val.item() if hasattr(layer_val, "item") else layer_val
            self.log(f"val/sr/{layer_name}", scalar_val, prog_bar=False, logger=True, on_step=True, on_epoch=True, sync_dist=True)
        for layer_name, layer_val in sr_yol_layers.items():
            scalar_val = layer_val.item() if hasattr(layer_val, "item") else layer_val
            self.log(f"val/sr_yol/{layer_name}", scalar_val, prog_bar=False, logger=True, on_step=True, on_epoch=True, sync_dist=True)
