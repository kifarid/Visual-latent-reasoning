...
class ModelREPA(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        context_noised, noise_ctx = self.add_noise_ctx(context, noise=None) if context is not None else (None, None)
        ...
        sr_loss_val, sr_layers = self.sr_loss(pred_latents, post_latents)
        sr_loss_mean = sr_loss_val.mean() if hasattr(sr_loss_val, "mean") else sr_loss_val
        ...
        aeloss, log_dict_ae = self.ae.loss(
            ...
            0, self.global_step, last_layer=self.ae.get_last_layer(), split="train"
        )

        loss_vae =  self.sr_loss_weight * sr_loss_mean + self.ae_loss_weight * aeloss
        ...
        sr_loss_mean = sr_loss_val.mean() if hasattr(sr_loss_val, "mean") else sr_loss_val
        # Total loss
        loss_fm = self.recon_loss_weight * v_loss_mean + self.sr_loss_weight * sr_loss_mean
        ...
        # ---------- Logging ----------
        total_loss = self.recon_loss_weight * v_loss_mean + self.sr_loss_weight * sr_loss_mean + self.ae_loss_weight * aeloss
        ...
        self.log("train/sr_loss", sr_loss_mean.item() if hasattr(sr_loss_mean, "item") else sr_loss_mean, prog_bar=True, logger=True, on_step=True, on_epoch=True, sync_dist=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        aeloss, log_dict_ae = self.ae.loss(
            ...
            images.reshape(b*f, *images.shape[2:]),
            x_rec.reshape(b*f, *x_rec.shape[2:]),
            0, self.global_step, last_layer=self.ae.get_last_layer(), split="val"
        )

        # ---------- (3) Combine losses ----------
        val_total = (
            self.recon_loss_weight * v_loss_mean
            + self.sr_loss_weight * sr_loss_mean
        ...
        self.log("val/sr_loss", sr_loss_mean.item() if hasattr(sr_loss_mean, "item") else sr_loss_mean, prog_bar=False, logger=True, on_step=False, on_epoch=True, sync_dist=True)
...
class ModelREPAIF(ModelREPA):
    ...
    def decode_frames(self, x, return_distill_conv_out=False):
        ...
        # Scale the two channel groups in one go: [B,T,C,H,W]
        x = torch.cat([
