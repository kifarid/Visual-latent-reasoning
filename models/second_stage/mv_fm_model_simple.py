...
class Model(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        pred = self.vit(model_input, t, t_indices=t_indices)
    ...
    def roll_out(self, x_0, num_gen_frames=25, latent_input=True, eta=0.0, NFE=20, sample_with_ema=True, num_samples=8):
        ...
        for idx in range(num_gen_frames):
            ...
            x_last_t = self.sample(images=x_c, latent=True, eta=eta, NFE=NFE, sample_with_ema=sample_with_ema, num_samples=num_samples)[0]
    ...
    @torch.no_grad()
    def sample(self, images=None, latent=False, eta=0.0, NFE=20, sample_with_ema=True, num_samples=8, t_indices=None, tgt_vector=None):
        '''
        ...
        with torch.no_grad():
            for i in range(NFE):
                ...
                neg_v = net(x_t, t=t * self.timescale, t_indices=t_indices)[tgt_vector]
    ...
    def log_images(self, batch, **kwargs):
        ...
        images, t_indices, tgt_vector = \
            images[:N], t_indices[:N], tgt_vector[:N]
        ...
        samples = self.sample(images, eta=0.0, NFE=30, sample_with_ema=False, num_samples=N, t_indices=t_indices)[1]
        ...
        samples_ema = self.sample(images, eta=0.0, NFE=30, sample_with_ema=True, num_samples=N, t_indices=t_indices)[1]
...
class ModelIF(Model):
    ...
    def training_step(self, batch, batch_idx):
        ...
        t[target_vector] = torch.rand((target.shape[0]//2,), device=x.device).repeat_interleave(2)
