...
#        assert len(x.shape) == 5, 'input must be 5D tensor'
    ...
    def training_step(self, batch, batch_idx):
        ...
        if tnk_enabled: 
            ...
            for i in range(self.num_thinking_steps):
                ...
                t_masked = torch.where(thinking_frames_mask[i], t, torch.zeros_like(t))
                frame_ids_tnk = torch.ones((b, f), device=x.device, dtype=torch.long) * (mask_sequence.shape[1] - 1 -  frame_ids["thinking"][i])
                target_t, (noise, _, _, _) = self.add_noise_ctx(target, t=t_masked.reshape(-1), mask=thinking_frames_mask[i].reshape(-1))
    ...
    def sample(self, images=None, mask=None, frame_ids=None, latent=False, eta=0.0, NFE=20, sample_with_ema=True, num_samples=8, frame_rate=None, return_sample=False):
        ...
        with torch.no_grad():
            for i in range(NFE):
                t = t_steps_masked[i]
                neg_v = net(target_t_mask, context, t=t * self.timescale, frame_idxs=frame_ids)
    ...
    def log_images(self, batch, **kwargs):
        ...
        # sample
        samples = self.sample(images_for_sample, mask=interm_masks, eta=0.0, NFE=30, frame_ids=frame_ids_ctx, sample_with_ema=False, num_samples=N, return_sample=True)[1]
        ...
        # sample
        samples_ema = self.sample(images_for_sample, mask=interm_masks, eta=0.0, NFE=30, frame_ids=frame_ids_ctx, sample_with_ema=True, num_samples=N, return_sample=True)[1]
        # Only keep the first generated frame
