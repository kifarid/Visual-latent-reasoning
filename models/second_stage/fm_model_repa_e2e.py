...
class ModelREPA(pl.LightningModule):
    def __init__(self, *, tokenizer_config, generator_config, adjust_lr_to_batch_size=False, sigma_min=1e-5, timescale=1.0, enc_scale=4, warmup_steps=5000, min_lr_multiplier=0.1,num_pred_frames=1, sr_latents=[],
        ...
        self.build_projector(generator_config.params.hidden_size, latent_dim=latent_dim)
    ...
    def add_noise_ctx(self, x, noise=None):
        ...
        b = x.shape[0]
        batch_mask = (torch.rand(b, device=x.device) < self.ctx_noise_prob).view(b, 1, 1, 1, 1)
    ...
    def sr_loss(self, pred_latents, post_latents):
        # Debug prints
        print("latents_shapes:")
        for i in range(len(pred_latents)):
            print(f"pred_latents[{i}]: {tuple(pred_latents[i].shape)}")
        print(f"post_latents_shapes: {tuple(post_latents.shape)}")

        # post is (B, F, N, D); each pred[i] is (B, F, C, H, W)
        Bp, Fp, N, D = post_latents.shape
        ...
        for i in range(len(pred_latents)):
            ...
            h_n = max(1, int(math.floor(math.sqrt(N))))
            w_n = max(1, int(math.ceil(N / h_n)))

            pred2d = pred.view(B * F_, C, H, W)
            pred2d = F.adaptive_avg_pool2d(pred2d, (h_n, w_n))          # (B*F, C, h_n, w_n)
            pred_seq = pred2d.view(B, F_, C, h_n * w_n).transpose(-1, -2)  # (B, F, N', C)
            ...
            if Np > N:
                ...
                pred_seq = F.pad(pred_seq, (0, 0, 0, N - Np))  # pad along N

            # project channels to D if needed
            if pred_seq.shape[-1] != D:
                if self.predictor_proj is None or self.predictor_proj[i] is None:
                    raise ValueError(f"Need predictor_proj[{i}] to map {pred_seq.shape[-1]} -> {D}")
                pred_seq = self.predictor_proj[i](pred_seq)    # (..., C)->(..., D)

            # optional norm
            if self.predictor_norm is not None and self.predictor_norm[i] is not None:
                pred_seq = self.predictor_norm[i](pred_seq)
                post = self.predictor_norm[i](post_latents)
            else:
                post = post_latents
            ...
            if self.loss_type in ["mse", "both"]:
                loss_unred = loss_unred + F.mse_loss(pred_seq, post.detach(), reduction='none')
            if self.loss_type in ["cos_sim", "both"]:
                cos = 1 - F.cosine_similarity(pred_seq, post.detach(), dim=-1, eps=1e-8)  # (B,F,N)
    ...
    def training_step(self, batch, batch_idx):
        ...
        pred, pred_latents = self.vit(target_t, context_noised, t, return_latents=self.return_latents) 
