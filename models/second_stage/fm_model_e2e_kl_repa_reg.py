...
class ModelREPA(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        opt_fm, opt_ae, _ = self.optimizers()
        sched_fm, sched_ae, _ = self.lr_schedulers()
        ...
        with torch.no_grad():
            ...
            _, post_latents_yol = self.ema_vit(target_ema, context_ema, t.pow(5), return_latents=self.sr_latents, early_exit=True)
        ...
        if (
            ...
            #and self._ctx_noise_log_epoch != self.current_epoch
        ):
            with torch.no_grad():
                ctx_frames = images[:, :-self.num_pred_frames] if f - self.num_pred_frames > 0 else None
                if ctx_frames is not None:
                    ctx_clean = ctx_frames.detach()
                    #add patching to 16 x16
                    #ctx_clean_patched, meta = self.patch_second_stage(ctx_clean, patch_size=(16, 16))
                    ctx_clean_patched = rearrange(ctx_clean, 'b f c (hp p1) (wp p2) -> b f (c p1 p2) hp wp', p1=16, p2=16, hp=ctx_clean.shape[3]//16, wp=ctx_clean.shape[4]//16)
                    print("Context clean shape:", ctx_clean.shape, "patched:", ctx_clean_patched.shape) #, "meta:", meta)
                    ctx_noised_patched, _ = self.add_noise_ctx(ctx_clean_patched.clone(), mask=mask_ctx, batch_mask=batch_mask_ctx, t=t_ctx, patch_size=patch_size_ctx)
                    ctx_clean = rearrange(ctx_clean_patched, 'b f (c p1 p2) hp wp -> b f c (hp p1) (wp p2)', p1=16, p2=16, hp=ctx_clean.shape[3]//16, wp=ctx_clean.shape[4]//16)
                    ctx_noised = rearrange(ctx_noised_patched, 'b f (c p1 p2) hp wp -> b f c (hp p1) (wp p2)', p1=16, p2=16, hp=ctx_clean.shape[3]//16, wp=ctx_clean.shape[4]//16)
                    ctx_clean = torch.clamp(ctx_clean, -1.0, 1.0)
                    ctx_noised = torch.clamp(ctx_noised, -1.0, 1.0)
                    clean_grid = vutils.make_grid(
                        rearrange(ctx_clean, 'b fr c h w -> (fr b) c h w'),
                        nrow=b,
                        padding=2,
                        normalize=False,
                    )
                    noised_grid = vutils.make_grid(
                        rearrange(ctx_noised, 'b fr c h w -> (fr b) c h w'),
                        nrow=b,
                        padding=2,
                        normalize=False,
                    )
                    try:
                        ...
                        self.logger.experiment.log(
                            {
                                "train/context_clean": wandb.Image(clean_grid.cpu()),
                                "train/context_noised": wandb.Image(noised_grid.cpu()),
                            },
                            step=self.global_step,
                        )
                        self._ctx_noise_log_epoch = self.current_epoch
                    except ImportError:
                        pass
        ...
        aeloss, log_dict_ae = self.ae.loss(
            ...
            0, self.global_step, #last_layer=self.ae.get_last_layer(),
              split="train"
        )
